...
export default function FreeShareList() {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useQuery({
    queryKey: ['freeshareList'],
    queryFn: () => getBoardList(1, 200, 0),
  });
  ...
  return (
    <div className={styles['list-container']}>
        ...
        <Table aria-label="simple table">
          ...
          <TableBody>
            {newData.slice((page - 1) * PER_PAGE, page * PER_PAGE).map((item, index) => (
              <TableRow
                component={Link}
                to={`/community/freesharedetail/${item.boardId}`}
                key={index}
      ...
      />
      <Link to="/community/freesharewrite">
        <button>글쓰기</button>
      </Link>
    </div>
  );
